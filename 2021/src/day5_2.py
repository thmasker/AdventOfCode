    ...
    for vector in vectors:
        if vector.is_horizontal():
            ...
            if vector.x1 > vector.x2:
                x_range = [x for x in range(vector.x1, vector.x2 - 1, -1)]
            else:
                x_range = [x for x in range(vector.x1, vector.x2 + 1)]

            if vector.y1 > vector.y2:
                y_range = [y for y in range(vector.y1, vector.y2 - 1, -1)]
            else:
                y_range = [y for y in range(vector.y1, vector.y2 + 1)]

            for i in range(len(x_range)):
                table[y_range[i]][x_range[i]] += 1
